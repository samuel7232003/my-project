    ...
    return (
        <div className={css.loginBackground}>
            <div className={css.loginContainer}>
                <h1>Login</h1>
                <p>Email/ Username</p>
                <Input placeholder="Enter your email or username" value={userName} onChange={(e) => setUserName(e.target.value)} />
                <p>Password</p>
                <Input.Password placeholder="Enter your password" value={password} onChange={(e) => setPassword(e.target.value)} />
                {error && <p className={css.error}>{error}</p>}
                <Button className={css.loginButton} type="primary" onClick={handleLogin}>Login</Button>
            </div>
        </div>
    );
}